...
def coqa_loss_fn( start_positions,
                  end_positions,
                  start_logits,
                  end_logits,
                  is_ynu,
    ...
    rt_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=tf.cast(rationale_tags,tf.float32),logits=rt_logits))

    total_loss = span_loss+rt_loss
...
def get_loss_fn(loss_factor=1.0):
  ...
  def _loss_fn(labels, model_outputs):
    ...
    is_ynu =tf.stack([labels['is_yes'],labels['is_no'],labels['is_unknown']],axis=1)
    ratinale_tags = labels['rationale_tags']
    #unique_ids,final_dists, attn_dists = model_outputs

    unique_ids,start_logits,end_logits,ynu_logits,rt_logits  = model_outputs
...
def get_raw_results(predictions):
  ...
  for unique_ids, start_logits, end_logits  in zip(predictions['unique_ids'],
      ...
      for values in zip(unique_ids.numpy(), start_logits.numpy(), end_logits.numpy()):
          yield coqa_span_rationale_tag_lib.RawResult(
              unique_id=values[0],
...
def predict_coqa_customized(strategy, input_meta_data, bert_config,
  ...
  with tf.device(primary_cpu_task):
    predict_dataset = input_pipeline.create_coqa_span_rationale_tag_dataset(
    ...
    with strategy.scope():
      ...
      coqa_model, _ = coqa_models.coqa_model_span_rationale_tag(
          config=bert_config,
          max_seq_length=input_meta_data['max_seq_length'],
    ...
    def predict_step(iterator):
      ...
      def _replicated_step(inputs):
          ...
          x, _ = inputs
          unique_ids, start_logits, end_logits,ynu_logits,_ = coqa_model(x, training=False)
...
def train_coqa(strategy,
  ...
  train_input_fn = functools.partial(
      input_pipeline.create_coqa_span_rationale_tag_dataset,
  ...
  def _get_coqa_model():
    ...
    coqa_model, core_model  = coqa_models.coqa_model_span_rationale_tag(
        bert_config,
        max_seq_length,
        max_answer_length,
  ...
  model_training_utils.run_customized_training_loop(
      ...
      init_checkpoint= FLAGS.init_checkpoint, #let's use BERT
...
def predict_coqa(strategy, input_meta_data):
  ...
  eval_examples = coqa_span_rationale_tag_lib.read_coqa_examples(
  ...
  eval_writer = coqa_span_rationale_tag_lib.FeatureWriter(
  ...
  # later on.
  dataset_size = coqa_span_rationale_tag_lib.convert_examples_to_features(
  ...
  coqa_span_rationale_tag_lib.write_predictions_bert_span(
      eval_examples,
      eval_features,
      all_results,
