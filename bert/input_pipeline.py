...
def create_coqa_dataset(file_path, seq_length, batch_size, is_training=True):
  """Creates input dataset from (tf)records files for train/eval."""
  name_to_features = {
      'unique_ids': tf.io.FixedLenFeature([], tf.int64),
      'input_ids': tf.io.FixedLenFeature([seq_length], tf.int64),
...
def create_coqa_span_rationale_tag_dataset(file_path, seq_length, batch_size, is_training=True):
  ...
  dataset = dataset.prefetch(1024)
  return dataset
def create_coqa_bert_rt_transformer_dataset(file_path, seq_length,answer_len, batch_size, is_training=True):
  """Creates input dataset from (tf)records files for train/eval."""
  name_to_features = {
      'unique_ids': tf.io.FixedLenFeature([], tf.int64),
      'input_ids': tf.io.FixedLenFeature([seq_length], tf.int64),
      ...
      'decode_mask': tf.io.FixedLenFeature([answer_len], tf.int64),
  }
  if is_training:
  ...
  def _select_data_from_record(record):
    ...
    x = {
        ...
        'decode_mask': record['decode_mask'],
    }
    for name, tensor in record.items():
       if name in ('answer_ids', 'answer_mask', 'rationale_tags' ):
                y[name] = tensor
