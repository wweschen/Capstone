...
import bert.bert_modeling as modeling
...
class BertCoqaLogitsLayer(tf.keras.layers.Layer):
  """Returns a layer that computes custom logits for BERT squad model."""

  def __init__(self, initializer=None, float_type=tf.float32, **kwargs):
    super(BertCoqaLogitsLayer, self).__init__(**kwargs)
    self.initializer = initializer
    self.float_type = float_type
  ...
  def build(self, unused_input_shapes):
    ...
    self.final_dense = tf.keras.layers.Dense(
        units=2, kernel_initializer=self.initializer, name='final_dense')
    super(BertCoqaLogitsLayer, self).build(unused_input_shapes)
...
def coqa_model(bert_config, max_seq_length, float_type, initializer=None):
  ...
  if initializer is None:
    initializer = tf.keras.initializers.TruncatedNormal(
        stddev=bert_config.initializer_range)
  coqa_logits_layer = BertCoqaLogitsLayer(
