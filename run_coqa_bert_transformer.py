...
def get_raw_results(predictions):
  ...
  for unique_id, sentence_ids  in zip(predictions['unique_ids'],
      ...
      yield coqa_bert_lib.RawResultEnd2end(
          unique_id=unique_id,
...
def predict_coqa_customized(strategy, input_meta_data, bert_config,
  ...
  with tf.device(primary_cpu_task):
    ...
    with strategy.scope():
      ...
      coqa_model, _ = coqa_models.coqa_model_bert_transformer(
          config=bert_config,
          max_seq_length=input_meta_data['max_seq_length'],
...
def train_coqa(strategy,
  ...
  def _get_coqa_model():
    ...
    coqa_model, core_model  = coqa_models.coqa_model_bert_transformer(
        bert_config,
        max_seq_length,
        max_answer_length,
...
def predict_coqa(strategy, input_meta_data):
  ...
  eval_examples = coqa_bert_lib.read_coqa_examples(
  ...
  eval_writer = coqa_bert_lib.FeatureWriter(
  ...
  # later on.
  dataset_size = coqa_bert_lib.convert_examples_to_features(
  ...
  coqa_bert_lib.write_predictions_end2end(
      eval_examples,
      eval_features,
      all_results,
...
def export_coqa(model_export_path, input_meta_data):
  ...
  coqa_model = coqa_models.coqa_model_bert_transformer(
      bert_config,
      input_meta_data['max_seq_length'],
      float_type=tf.float32)
