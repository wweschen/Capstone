...
def coqa_loss_fn( final_dists,
    ...
    total_loss = _loss+loss_factor* rt_loss
...
def get_loss_fn(loss_factor=1.0):
  ...
  def _loss_fn(labels, model_outputs):
    ...
    unique_ids,final_dist, rt_logits  = model_outputs


    return coqa_loss_fn(final_dist,
                        target_words_ids,
                        target_words_mask,
                        ratinale_tags,
                        rt_logits,
...
def predict_coqa_customized(strategy, input_meta_data, bert_config,
  ...
  with tf.device(primary_cpu_task):
    ...
    with strategy.scope():
      ...
      coqa_model, _ = coqa_models.coqa_model_bert_rt_transformer(
          config=bert_config,
          max_seq_length=input_meta_data['max_seq_length'],
    ...
    def predict_step(iterator):
      ...
      def _replicated_step(inputs):
          ...
          x, _ = inputs
          unique_ids, final_dists,_ = coqa_model(x, training=False)
    ...
    def decode_sequence(x):
        ...
        for i in range(1, bert_config.max_answer_length):
            ...
            next_pred = tf.argmax(dists, axis=-1, output_type=tf.int32)
...
def train_coqa(strategy,
  ...
  train_input_fn = functools.partial(
      input_pipeline.create_coqa_bert_rt_transformer_dataset,
  ...
  def _get_coqa_model():
    ...
    coqa_model, core_model  = coqa_models.coqa_model_bert_rt_transformer(
        bert_config,
        max_seq_length,
        max_answer_length,
...
def predict_coqa(strategy, input_meta_data):
  ...
  eval_examples = coqa_bert_rt_transformer_lib.read_coqa_examples(
  ...
  eval_writer = coqa_bert_rt_transformer_lib.FeatureWriter(
  ...
  # later on.
  dataset_size = coqa_bert_rt_transformer_lib.convert_examples_to_features(
...
def export_coqa(model_export_path, input_meta_data):
  ...
  coqa_model = coqa_models.coqa_model_bert_rt_transformer(
      bert_config,
      input_meta_data['max_seq_length'],
      float_type=tf.float32)
