...
class InputFeatures(object):
    ...
    def __init__(self,
        ...
        self.is_no = is_no
        self.is_unknown = is_unknown
        self.rationale_tags = rationale_tags
...
class FeatureWriter(object):
    ...
    def process_feature(self, feature):
        ...
        if self.is_training:
            ...
            features['rationale_tags'] = create_int_feature(feature.rationale_tags)
    ...
    for (example_index, example) in enumerate(examples):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if is_training:
                ...
                if is_unknown==1 or (end_position <0 or start_position<0):
                    rationale_tags = [0] * max_seq_length
                else:
                    rationale_tags = [0] * start_position

                    rationale_tags.extend([1] * (end_position - start_position))
                    rationale_tags.extend([0] * (max_seq_length - len(rationale_tags)))

                if is_yes==1 or is_no==1 or is_unknown==1:
                    start_position = -1
                    end_position = -1

                assert len(rationale_tags) == max_seq_length
            ...
            if example_index < 50 :
                ...
                if is_training:
                    ...
                    logging.info(
                        "rationale tags: %s" % " ".join([str(x) for x in rationale_tags]))
                    logging.info(
...
def generate_tf_record_from_json_file(input_file_path,
    ...
    meta_data = {
        "task_type": "coqa_span_rationale_tag",
